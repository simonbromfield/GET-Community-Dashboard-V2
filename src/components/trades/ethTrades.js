...
const EthTrades = (props) => {
  ...
  const ethSushiSubGraphURL = 'https://api.thegraph.com/subgraphs/name/sushiswap/exchange'

  const recentEthTradesFunction = async () => {
    try {
      await axios.post(ethSushiSubGraphURL, {
        query: `
        {
  ...
  function displayRecentEthTrades() {
    return (<>
      ...
      <CardHeader
        title="SushiSwap - Mainnet - Most recent 20"
        />
        <TableContainer>
            <Table >
              <TableHead>
...
return (
  <>
    {loading ? displayRecentEthTrades() :
      <LoadingSVG />
    }
  </>
)
