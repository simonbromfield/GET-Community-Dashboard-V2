...
const PolyTrades = (props) => {
  ...
  const polySushiSubGraphURL = 'https://api.thegraph.com/subgraphs/name/sushiswap/matic-exchange'

  const recentPolyTradesFunction = async () => {
    try {
      await axios.post(polySushiSubGraphURL, {
        query: `
        {
  ...
  function displayRecentPolyTrades() {
    return (<>
      ...
      <CardHeader
        title="SushiSwap - MATIC (Polygon) - Most recent 20"
        />
        <TableContainer>
        <Table >
          <TableHead>
...
return (
  <>
    {loading ? displayRecentPolyTrades() :
        <LoadingSVG />
    }
  </>
)
