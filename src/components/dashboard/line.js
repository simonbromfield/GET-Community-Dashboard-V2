...
const TicketsSoldApp = (props) => {
  ...
  const [protocolDaysRange, setProtocolDaysRange] = useState(protocolDays.slice(0,value))
  const [sold, setSold] = useState(protocolDaysRange.map(x => x.soldCount));
  const [reSold, setReSold] = useState(protocolDaysRange.map(x => x.resoldCount));
  const [scanned, setScanned] = useState(protocolDaysRange.map(x => x.scannedCount));
  const [days, setDays] = useState(protocolDaysRange.map(x => {
    let dayString;
  ...
  useEffect(() => {
    ...
    setDays(protocolDaysRange.map(x => {
      let dayString;
      dayString = protocolDayToDate(x.day)
      return dayString;
    }));
